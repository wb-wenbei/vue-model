...
export default {
  ...
  data() {
    return {
      ...
      searchColumns: [
        { prop: "entityName", label: "物业名称", type: "input" },
        {
      ...
      headers: [
        ...
        { prop: "entityDirectorName", label: "物业负责人" },
        { prop: "contactPhone", label: "联系电话" },
        { prop: "address", label: "物业地址" },
  ...
  methods: {
    getOptions() {
      this.modelUrl =
        "/api-customer/service-wisdom-town/property/download/excel?token=" +
    ...
    deleteRow(row) {
      this.$confirm("确定要删除吗？").then(() => {
        deleteAPI({ id: row.id })
          .then(() => {
            this.$message.success("物业删除成功！");
            this.$refs.table.refresh();
          })
          .catch(err => {
            this.$message.error("物业删除失败：" + err);
          });
      });
    },
    add() {
